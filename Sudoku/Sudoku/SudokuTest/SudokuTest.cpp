...
{		
	...
	public:
        ...
        //检测能否设置元素测试
        TEST_METHOD(CanSetMatrix)
        {
            Sudoku sudo;
            Assert::IsTrue(sudo.CanSetMartrix(1, 1, 9));
            Assert::IsTrue(sudo.CanSetMartrix(2, 2, 9));
            Assert::IsTrue(sudo.CanSetMartrix(1, 8, 9));
            Assert::IsTrue(sudo.CanSetMartrix(8, 1, 9));
            sudo.SetMatrix(1, 1, 9);
            Assert::IsFalse(sudo.CanSetMartrix(2, 2, 9));
            Assert::IsFalse(sudo.CanSetMartrix(1, 8, 9));
            Assert::IsFalse(sudo.CanSetMartrix(8, 1, 9));

            sudo = Sudoku();
            Assert::IsTrue(sudo.CanSetMartrix(4, 4, 9));
            Assert::IsTrue(sudo.CanSetMartrix(2, 2, 4));
            sudo.SetMatrix(1, 1, 9);
            Assert::IsTrue(sudo.CanSetMartrix(4, 4, 9));
            Assert::IsTrue(sudo.CanSetMartrix(2, 2, 4));
        }
		...
		TEST_METHOD(GetAndSetMatrix)
		{
            ...
            {
                std::pair<int, int> pos = point[i].first;
